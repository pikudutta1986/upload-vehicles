		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if (isset($_POST['country_create']))
			{
				...
				if ($id !== false && (int) $id > 0)
				{
					...
					if (isset($_POST['i18n']))
					{
						pjMultiLangModel::factory()->saveMultiLang($_POST['i18n'], $id, 'pjCountry', 'data');
					}
				} else {
					$err = 'AC04';
				}
	...
	public function pjActionSaveCountry()
	{
		...
		if ($this->isXHR())
		{
			...
			if (!in_array($_POST['column'], $pjCountryModel->i18n))
			{
				...
				pjMultiLangModel::factory()->updateMultiLang(array($this->getLocaleId() => array($_POST['column'] => $_POST['value'])), $_GET['id'], 'pjCountry', 'data');
			}
		}
		exit;
	}
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isAdmin() || $this->isEditor())
		{
			if (isset($_POST['country_update']))
			{
				...
				if (isset($_POST['i18n']))
				{
					pjMultiLangModel::factory()->updateMultiLang($_POST['i18n'], $_POST['id'], 'pjCountry', 'data');
				}
