...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminCarModels&amp;action=pjActionCreate" method="post" id="frmCreateCarModel" class="pj-form form">
		<input type="hidden" name="model_create" value="1" />
		...
		<div class="clear_both">
			...
			<p>
				...
				<input type="button" value="<?php __('btnCancel'); ?>" class="pj-button" onclick="window.location.href='<?php echo PJ_INSTALL_URL; ?>index.php?controller=pjAdminCarModels&action=pjActionIndex';" />
			</p>
		</div>
	</form>
			...
			$(".multilang").multilang({
				...
				select: function (event, ui) {
					...
					$.get("index.php?controller=pjAdminCarModels&action=pjActionGetLocale", {
						...
						mid = $("#make_id").find("option:selected").val();
