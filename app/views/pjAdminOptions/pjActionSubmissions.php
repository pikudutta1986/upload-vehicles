...
if (isset($tpl['status']))
{
	...
	if (isset($tpl['arr']))
	{
		if (is_array($tpl['arr']))
		{
			...
			if ($count > 0)
			{
									...
									switch ($tpl['arr'][$i]['key'])
									{
										case 'o_paypal_address':
											...
											<span class="pj-form-field-custom pj-form-field-custom-before">
												...
												<input type="text" name="value-<?php echo $tpl['arr'][$i]['type']; ?>-<?php echo $tpl['arr'][$i]['key']; ?>" class="pj-form-field w200 email" value="<?php echo htmlspecialchars(stripslashes($tpl['arr'][$i]['value'])); ?>" />
											</span>
											<?php
											break;
										default:
				...
				?>
				<tr class="pj-table-row-odd">
					<td colspan="3"><?php pjUtil::printNotice(null, __('opt_period_note', true)); ?></td>
				</tr>
				<tr class="pj-table-row-even">
					<td class="bold"><?php __('car_payment_period'); ?></td>
					<td class="bold" colspan="2"><?php __('car_payment_price'); ?></td>
				</tr>
				<?php
				...
				<div id="dialogDeletePeriod" style="display: none" title="<?php __('cl_delete_period_title');?>"><?php __('cl_delete_period_body');?></div>
				<?php
			}
		}
	}
	...
	myLabel.email_invalid = "<?php __('cl_email_invalid'); ?>";
	</script>
	<?php
}
?>