...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminFeatures&amp;action=pjActionUpdate" method="post" id="frmUpdateFeature" class="form pj-form">
			...
			<p>
				...
				<span class="inline_block" id="boxType">
					<select name="type" id="type" class="pj-form-field w200 required">
						...
						{
							?><option value="<?php echo $k;?>"<?php echo $k == $tpl['arr']['type'] ? ' selected="selected"' : NULL;?>><?php echo stripslashes($v);?></option><?php
						} 
						?>
					</select>
				</span>
			...
			$(".multilang").multilang({
				...
				select: function (event, ui) {
					...
					$.get("index.php?controller=pjAdminFeatures&action=pjActionGetLocale", {
						...
						type = "<?php echo !empty($tpl['arr']['type']) ? $tpl['arr']['type'] : ''; ?>";
