...
(function ($, undefined) {
	...
	function Gallery() {
		...
		this.messages = {
			...
			drag_drop_sub_title: "to upload new image",
			edit: "Edit",
	...
	Gallery.prototype = {
				...
				.dialog({
					...
					open: function () {
						...
						$.get(inst.settings.updateUrl, {
							...
							var $p = $("<p>");
							$("<label>").addClass("title").text(self.messages.title).appendTo($p);
							$("<input>", {
								"type": "text",
								"name": "title",
								"value": data.title
							...
							$p = $("<p>");
							$("<label>").addClass("title").text(self.messages.description).appendTo($p);
							$("<textarea>", {
								"type": "text",
								"name": "description",
								"value": data.description
							}).addClass("pj-form-field w300 h80").appendTo($p);
							$p.appendTo($this);
		...
		_renderGallery: function (target) {
			...
			var uploadApi = new AjaxUpload('pj-gallery-upload-' + inst.uid, {
				...
				onSubmit: function(file, extension) {
					...
					$target.find(".pj-gallery-upload").val(self.messages.uploading);
				},
				...
				onComplete: function(file, response) {
					...
					uploadApi.enable();
					$target.find(".pj-gallery-upload").val(self.messages.upload);
					var qs = '';
			...
			function readfiles(files) {
				...
				$.ajax({
					...
					if (data.error !== undefined && data.error.length > 0) {
						qs = inst.settings.getUrl.indexOf("?") !== -1 ? "&error=" + data.error : "?error=" + data.error;
					}
					self._loadGallery.call(self, target, inst.settings.getUrl + qs);
				});
			}
			...
			if (tests.dnd) {
				var $dd = $("<div>").addClass("pj-gallery-droppable");
				if (inst.settings.content.data.length === 0) {
					$dd.addClass("pj-gallery-droppable-full");
				}
				$('<div>').addClass('pj-gallery-droppable-1').text(self.messages.drag_drop_title).appendTo($dd);
				$('<div>').addClass('pj-gallery-droppable-2').text(self.messages.drag_drop_sub_title).appendTo($dd);
				$target.append($dd);
				var holder = $dd.get(0);
				
				holder.ondragover = function () {
					$(this).addClass('pj-gallery-droppable-hover');
					return false;
				};
				holder.ondragend = function () {
					$(this).removeClass('pj-gallery-droppable-hover');
					return false;
				};
